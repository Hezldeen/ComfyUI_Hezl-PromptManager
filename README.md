...
### 260825
1. CSV 文件编码从 `utf-8` 改为 `utf-8-sig`(带 BOM),解决 Excel 打开 CSV 时中文乱码的问题.

### 260820
1. 批量移动词组弹窗新增**目标CSV搜索功能**:在"选择目标CSV文件"下拉框上方新增搜索输入框,输入关键词实时过滤CSV文件路径(不区分大小写,`/`与`\`视为相同分隔符),下方显示"匹配 X / Y 个CSV文件"计数;当前选中项被过滤隐藏时自动切换到第一个匹配项,无匹配时阻止误操作;打开弹窗即自动聚焦搜索框.
2. 修复批量移动词组确认后**误报"移动失败"的BUG**:根因是清理词组栏引用时遍历 `bars` 未跳过文本框条目(文本框没有 `prompts` 数组导致报错被计入失败数,词组实际已移动成功).现遍历时跳过非词组栏条目.
3. 批量移动改用新增的后端原子接口 `/hezl_prompt/batch_move_prompts`:单次请求完成"写入目标CSV+从源CSV移除"(原先逐词组发两次请求,中途失败会出现词组被**复制而非移动**且不报错的中间态);写入顺序为先目标后源,中途失败只会复制不会丢失;返回逐词组的成功/失败明细,同名冲突等原因直接显示在提示中.
4. 批量移动成功后,**词组栏中已引用的词组保留且来源路径自动更新为目标CSV**(原先会从词组栏中移除引用,与"移动CSV文件"功能的行为保持一致).
5. 修复其余 4 处同源的文本框遍历报错BUG:批量删除词组、移动CSV文件、重命名CSV文件、删除文件夹时,同样因未跳过文本框条目导致误报"删除/移动/重命名失败"(操作实际已成功,但后续界面刷新被中断);批量删除词组同时补上了对删除接口返回结果的检查,删除失败时不再错误地清理词组栏引用.

### 260815
1. 修复删除 CSV 文件时提示"删除失败: Cannot read properties of undefined (reading 'filter')"的 BUG:此前遍历 `bars` 时未跳过无 `prompts` 数组的文本框条目,导致报错(文件实际已删除)。现删除文件前先判断条目类型,跳过文本框,使 CSV 文件能正常删除。
2. "编辑词组"与"新建词组"弹窗优化:
   - 默认尺寸加大为 480×420(原先受限 `max-width: 340px` 且固定大小),并支持在**右边缘/下边缘/右下角**拖拽边界调整大小(最小 320×240,不超出视口)。
   - 弹窗内文本框(flex 布局)会**随窗口大小同步伸缩**。
   - 应用到编辑词组 / 添加词组 / 在⬅/➡添词组 三个弹窗。
3. 修复"词组栏选择预设后,右键词组点击编辑无法打开编辑窗口"的 BUG:原编辑逻辑只在当前文件夹 `promptsData` 中查找词组,词组栏里的词组(可能来自其他文件夹/预设)查找不到导致直接返回。现右键菜单直接传入完整词组对象,编辑弹窗不再依赖查找,保存后正确写回原 CSV。
